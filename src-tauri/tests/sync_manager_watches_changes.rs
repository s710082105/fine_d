...
fn build_config(source_root: &PathBuf) -> ProjectConfig {
  ProjectConfig {
    ...
    workspace: WorkspaceProfile {
      name: "demo".into(),
      root_dir: source_root.parent().expect("source root parent").display().to_string(),
    },
    sync: SyncProfile {
...
fn sync_manager_watches_local_changes_and_emits_sync_events() {
  ...
  loop {
    ...
    if tasks.iter().any(|task| task.remote_path.ends_with("reportlets/report.cpt")) {
      break;
    }
    drop(tasks);
    attempts += 1;
  ...
  assert!(events.iter().any(|event| event.event_type == finereport_tauri_shell_lib::domain::event_bridge::SessionEventType::Sync));
  assert!(events.iter().any(|event| event.sync_path.as_deref() == Some("/srv/tomcat/webapps/webroot/WEB-INF/reportlets/report.cpt")));
}
