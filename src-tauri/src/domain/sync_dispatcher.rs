...
impl SyncManager {
  ...
  pub fn watch_session(
    ...
    let watch_root = PathBuf::from(&config.sync.local_source_dir);
    if !watch_root.exists() {
      return Err(format!(
        "local_source_dir does not exist: {}",
        watch_root.display()
      ));
    }
...
pub fn resolve_sync_task(
  ...
  let source_root = PathBuf::from(&config.sync.local_source_dir);
  let relative_path = local_path
    .strip_prefix(&source_root)
    .map_err(|_| format!("path is outside local_source_dir: {}", local_path.display()))?;
...
fn sync_snapshot_diff(
  ...
  for (path, modified) in current {
    ...
    if let Some(action) = action {
      apply_sync_change(session_id, config, bridge, transport, path.as_path(), action);
    }
  }
  for path in previous.keys() {
    ...
    apply_sync_change(session_id, config, bridge, transport, path.as_path(), SyncAction::Delete);
  }
}
