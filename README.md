...
- 通过中文浏览器界面选择当前项目目录，并按项目维度保存远程参数。
- 在同一页面查看远程目录和远程数据连接。
- 在独立页面承载 Codex 终端会话，并显式展示工作目录和会话状态。
...
- `project`：项目目录选择、项目级远程参数保存、当前项目状态读取
- `remote`：远程目录与数据连接统一概览
- `codex_terminal`：Codex 终端会话创建、轮询、输入写入与关闭
...
- `项目与远程概览`：选择目录、维护远程参数、查看远端概览
- `Codex`：直接承载终端会话，不自研聊天气泡或消息协议
