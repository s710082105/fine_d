...
4. 在项目目录执行 `git add`、`git commit`
5. 等待宿主完成同步并自动进入浏览器复核
6. 修改完成后汇报变更内容
...
- 打开预览页后，如果页面有查询按钮、参数面板或其他查询触发动作，必须先完成查询。
- 只有在页面已经出现实际数据结果后，才能检查列名、数据、排序、分页和样式。
- 没有查询结果时，不能凭空猜测“列应该是对的”或“数据应该已经出来了”。
- 如果查询后仍然没有结果，要先判断是参数、数据源、权限还是模板问题，再继续修改。
